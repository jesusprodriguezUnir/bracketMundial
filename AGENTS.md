...
- Reporte de fotos faltantes: `npm run ucl:report` (o `npm run assets:report`) → genera `docs/missing-assets.md`
...
- [src/data/ucl-2027.ts](src/data/ucl-2027.ts) define los 36 clubes UCL y fixtures base.
- [src/data/squads/](src/data/squads/) — un `.ts` por club con `Player[]` y `Lineup`. Registro central en `index.ts`.
...
- [scripts/fetch-ucl-squads.mjs](scripts/fetch-ucl-squads.mjs) — descarga plantillas, fotos oficiales, DTs y escudos desde UEFA.com. Modos: `--report`, `--photos`, `--data`, `--crests`, `--force`.
...
- Al realizar force-push manual a ramas de datos estáticos (`news-data`, `odds-data`), es MANDATORIO generar un `vercel.json` con formato válido de omisión de compilación. En PowerShell, usa here-strings (`@' ... '@`) para evitar que las comillas sean removidas o interpretadas de manera incorrecta por el intérprete de comandos, lo cual causaría errores de despliegue en Vercel ("Invalid vercel.json"). Además, el archivo DEBE escribirse en **UTF-8 sin BOM**; PowerShell puede insertar un BOM invisible (`U+FEFF`) que invalida el JSON. Para evitarlo, usa `[System.IO.File]::WriteAllText($path, $content, [System.Text.UTF8Encoding]::new($false))` en lugar de `Set-Content` o redirección `>`.
