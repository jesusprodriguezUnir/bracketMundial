...
if (ODDS_API_KEY) {
  ...
  for (const event of events) {
    ...
    matches[lookup.matchId] = { home, draw, away, bookmakers: probs.length, source: 'market' };
  }
...
if (WRITE_SEED) {
  ...
  const matchEntries = Object.entries(matches)
    .map(([id, o]) =>
      `  ${JSON.stringify(id)}: { home: ${o.home}, draw: ${o.draw}, away: ${o.away}, bookmakers: ${o.bookmakers}, source: '${o.source}' as const },`
    )
    .join('\n');
