...
export class MobileBracket extends LitElement {
  ...
  @state() private _awardsModal: 'topScorer' | 'mvp' | null = null;
  @state() private _awardsQuery = '';
  ...
  connectedCallback() {
    ...
    this._unsubAwards = subscribeSlice(
      useTournamentStore,
      s => `${s.myTopScorerPrediction?.playerName ?? ''}|${s.myMvpPrediction?.playerName ?? ''}`,
      () => this.requestUpdate(),
    );
  }
  ...
  // ─── Premios individuales (goleador / MVP) ───

  /** true si la liga activa está congelada: predicciones no editables. */
  private get _awardsFrozen(): boolean {
  ...
  private _openAwards(type: 'topScorer' | 'mvp') {
    this._awardsModal = type;
    this._awardsQuery = '';
  }

  private _closeAwards() {
    this._awardsModal = null;
    this._awardsQuery = '';
  }

  private _pickAwardPlayer(teamId: string, playerName: string) {
    const store = useTournamentStore.getState();
    const val = { teamId, playerName };
    if (this._awardsModal === 'topScorer') store.setMyTopScorerPrediction(val);
  ...
  private _renderAwards() {
    ...
    const frozen = this._awardsFrozen;
    ...
    const card = (icon: string, label: string, value: { teamId: string; playerName: string } | null, type: 'topScorer' | 'mvp') => html`
      <div class="award-card">
        ...
        ${frozen
          ...
          : html`<button class="award-btn" @click="${() => this._openAwards(type)}">${value ? t('knockout.change') : t('knockout.select')}</button>`}
      </div>
    `;
  ...
  private _renderAwardsModal() {
    if (!this._awardsModal) return html``;
    const locale = useLocaleStore.getState().locale;
    const typeLabel = this._awardsModal === 'topScorer'
      ? t('knockout.topScorerLabel').toUpperCase()
      : t('knockout.mvpLabel').toUpperCase();

    const query = this._awardsQuery.trim();
      ...
      body = html`
        ...
        <div class="awm-list">
          ${displayed.map(p => {
            ...
            return html`
              <button class="awm-player" @click="${() => this._pickAwardPlayer(p.teamId, p.name)}">
    ...
    return html`
      <div class="awm-backdrop" @click="${this._closeAwards}">
        <div class="awm-sheet" @click="${(e: Event) => e.stopPropagation()}">
          <div class="awm-header">
            ...
            <button class="awm-close" @click="${this._closeAwards}" aria-label="${locale === 'es' ? 'Cerrar' : 'Close'}">✕</button>
          </div>
          <div class="awm-search">
            <input
              type="search"
              .value="${this._awardsQuery}"
              @input="${(e: InputEvent) => { this._awardsQuery = (e.target as HTMLInputElement).value; }}"
